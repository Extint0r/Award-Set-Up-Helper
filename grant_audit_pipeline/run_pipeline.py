...
def parse_ai_header_payload(
    ...
    # 1. FAST PATH: Return cached JSON from disk if already processed (with cache sanitization applied)
    if json_cache_path.exists():
        try:
            with open(json_cache_path, "r", encoding="utf-8") as jf:
                cached_data = json.load(jf)
            
            # Pass cached identifiers through sanitization so legacy cached files auto-correct
            raw_idents = cached_data.get("IDENTIFIERS") or {}
            cached_data["IDENTIFIERS"] = sanitize_ai_identifiers(raw_idents, filename=pdf_path.name)
