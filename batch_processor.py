...
from ai_engine import client, extract_award_data, clean_json_text
...
def initialize_local_ledger_database():
    ...
    # 1. Table mapping standalone, unchangeable document map extractions
    cursor.execute("""
    ...
    # 2. Table mapping compiled versioned snapshots of the grouped reduce evaluations
    cursor.execute("""
...
def safe_int(val, default=0):
    """Safely converts string numeric envelopes into integers."""
    if val is None:
        return default
    clean = str(val).strip()
    try:
    ...
    for proposal_id, file_packet in groups.items():
        if proposal_id == "UNASSIGNED_PACKET":
            for raw_file in file_packet:
                log_administrative_error(raw_file, "Naming Rule Violation", "No proposal tracking mask found.")
            continue
            
        print(f"\n📦 Processing Ledger Stack for Proposal Packet: {proposal_id}")
        packet_updated = False
        
        # 1. 🗺️ THE RESRESILIENT EVENT MAP PHASE
        for filepath in file_packet:
            fname = os.path.basename(filepath)
            
            # Generate immutable file fingerprint hash
            with open(filepath, "rb") as f:
                file_bytes = f.read()
                f_hash = hashlib.sha256(file_bytes).hexdigest()
                
            # Query if this individual extraction already exists in the ledger
            cursor = db_conn.cursor()
            cursor.execute("SELECT id FROM individual_file_logs WHERE file_hash = ?", (f_hash,))
            existing_row = cursor.fetchone()
            
            if existing_row:
                # File already processed securely. Skip the AI layer completely.
                continue
                
            print(f"  📄 Ingesting New Standalone Transaction File: {fname}")
            packet_updated = True
            
            try:
                # Run standalone map extraction
                doc_json = extract_award_data(filepath)
                exec_date = parse_mixed_date(doc_json.get("document_execution_date", "N/A"))
                
                # Fetch next ascending sequence number for this proposal group
                cursor.execute("""
                ...
                # Commit to the unchangeable individual log ledger
                cursor.execute("""
        ...
        # Fetch all individual log logs currently committed for this proposal to compute version state
        cursor = db_conn.cursor()
        cursor.execute("""
            SELECT sequence_number, document_execution_date, filename, extraction_json 
            FROM individual_file_logs 
            WHERE proposal_id = ?
        """, (proposal_id,))
        db_rows = cursor.fetchall()
        
        if not db_rows:
            print(f"  ℹ️ Timeline current. No active logs found for {proposal_id}.")
            continue
            
        # Re-verify chronological order using execution date
        mapped_documents = []
        max_seq_included = 0
        ...
        # Check if a snapshot with this max sequence number already has a generated version row
        cursor.execute("""
        ...
        if existing_version and not packet_updated:
            print(f"  ℹ️ Snapshot current. Version index #{existing_version[0]} matches current state.")
            continue

        print(f"  ⚙️ Executing Reduce Engine on {len(mapped_documents)} events to build clean timeline...")
        ...
        for idx, doc in enumerate(mapped_documents):
            ...
            delta_dir = safe_float(doc.get("direct_funding_delta"))
            delta_ind = safe_float(doc.get("indirect_funding_delta"))
            delta_tot = safe_float(doc.get("total_funding_delta"))

            cum_direct += delta_dir
            cum_indirect += delta_ind
            cum_total += delta_tot
            ...
            for t in str(doc.get("subject_to_terms_and_conditions", "")).split(";"):
                if t.strip(): all_terms.add(t.strip())
            for f in str(doc.get("far_clauses", "")).split(";"):
            ...
            amendment_history.append({
                ...
                "cumulative_obligated_total": cum_total,
                "funding_delta_anticipated": delta_tot,
                "cumulative_anticipated_total": cum_total,
                "scope_or_terms_summary": f"Sequence entry #{doc['_db_sequence']} loaded via: {fname}"
            })
        ...
        # Commit final accumulators
        synthesized_json["start_date"] = final_start_date
        synthesized_json["end_date"] = final_end_date
        synthesized_json["direct_funding_obligated"] = cum_direct
        synthesized_json["indirect_funding_obligated"] = cum_indirect
        synthesized_json["total_funding_obligated"] = cum_total
        synthesized_json["direct_funding_total_awarded"] = cum_direct
        synthesized_json["indirect_funding_total_awarded"] = cum_indirect
        synthesized_json["total_funding_total_awarded"] = cum_total
        ...
        synthesized_json["discrepancy_summary"] = "No structural discrepancies detected across ledger stream."

        registry_row = master_registry_cache.get(str(proposal_id), {})
        
        # 3. 🤖 TEXT-ONLY SYNTHESIS GENERATION
        try:
            print("    🧠 Invoking text-only narrator summary for compliance analysis...")
            ...
            synthesis_res = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction="You are a senior research compliance auditor. Synthesize timelines into clean audit summaries. Output valid JSON only.",
            ...
            synthesized_json["evidentiary_justification"] = f"Automated ledger compilation completed. Narrative generator error: {synth_fail}"

        # 4. RUN THREE-WAY DELTA CALCULATIONS
        ...
        # Fetch next ascending version number for this proposal group
        cursor.execute("""
            SELECT COALESCE(MAX(version_number), 0) + 1 
            FROM reduce_version_logs WHERE proposal_id = ?
        """, (proposal_id,))
        next_ver = cursor.fetchone()[0]
        
        # Append the calculated versioned state to the reduce ledger registry
        cursor.execute("""
        ...
        db_conn.commit()
        print(f"    💾 Saved Unified Snapshot -> Snapshot Version #{next_ver} (Tracks Events up to Sequence #{max_seq_included})")
        
        # Stream the finalized versioned dataset down to the target Google Sheet workbook tabs
        push_extracted_data_to_sheets(TARGET_SPREADSHEET_ID, synthesized_json, system_uid, oracle_award_number)
        print(f"  ✅ Chronological Map-Reduce committed successfully to sheets for UID: {system_uid}")
